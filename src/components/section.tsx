...
function SectionTitle({className, ...props}:SectionTitleProps) {
  return (
    <span
      className={twMerge("bg-navy-700 rounded-lg px-3 py-1.5 items-center gap-2 text-xs", className)}
      {...props}
    />
  );
}
...
function SectionIssueCount({className, ...props}:SectionIssueCountProps) {
  return <span className={twMerge("text-xs text-gravy-200", className)} {...props} />;
}
